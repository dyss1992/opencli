...
async function handleCommand(cmd) {
	...
	try {
		switch (cmd.action) {
			...
			case "native-click": return await handleNativeClick(cmd, workspace);
...
async function handleNativeClick(cmd, workspace) {
	if (typeof cmd.x !== "number" || typeof cmd.y !== "number") return {
		id: cmd.id,
		ok: false,
		error: "Missing native click coordinates"
	};
	await nativeClick(await resolveTabId(cmd.tabId, workspace), cmd.x, cmd.y);
	return {
		id: cmd.id,
		ok: true,
		data: "clicked"
